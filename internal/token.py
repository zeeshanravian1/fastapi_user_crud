...
def create_access_token(data: dict, token_type: str) -> str:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Something went wrong while creating token",
        ) from err
...
def assign_scopes(scopes) -> List[str]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Something went wrong while assigning scopes",
        ) from err
...
async def get_current_user(
    ...
    query = (
        UserTable.join(RoleTable, RoleTable.c.id == UserTable.c.role_id)
        .select()
        .with_only_columns([UserTable, RoleTable.c.role_name])
...
async def get_current_active_user(
    current_user: UserReadSchema = Security(get_current_user),
) -> Dict:
    """
