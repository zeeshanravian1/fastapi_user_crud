...
from core.schemas.password_schemas import (
    PasswordSchema,
...
async def create_user(record: UserCreateSchema) -> dict:
    ...
    try:
        ...
        email_query = UserTable.select().where(
            UserTable.c.email == record.email
        )
        ...
        username_query = UserTable.select().where(
        ...
        otp_expiry_time = datetime.now() + timedelta(
            minutes=OTP_CODE_EXPIRY_MINUTES
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Unique key constraint fails with {err_message}",
        ) from err
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"Foreign key constraint fails with {err_message}",
        ) from err
...
async def set_password(record: PasswordSchema) -> dict:
    ...
    try:
        ...
        email_query = UserTable.select().where(
            UserTable.c.email == token_data["email"]
        )
        email_result = await database.fetch_one(query=email_query)
        ...
        query = (
            UserTable.update()
            .where(UserTable.c.email == token_data["email"])
            .values(
...
async def set_role(
    record: UserRoleCreateSchema,
    current_user: UserReadSchema = Security(
        get_current_active_user, scopes=[Role.SUPER_ADMIN["name"]]
    ),
) -> dict:
    """
    Sets role of user.
    ...
    try:
        ...
        query = (
            UserTable.update()
            .where(UserTable.c.id == record.user_id)
...
async def get_user(
    user_id: int,
    current_user: UserReadSchema = Security(
        get_current_active_user, scopes=[Role.REPORTING_USER["name"]]
    ),
) -> dict:
    """
    Get a single user.
...
async def update_user(
    ...
    try:
        ...
        if record.username:
            ...
            # Check if username already exists
            username_query = UserTable.select().where(
                UserTable.c.username == record.username
            )
            username_result = await database.fetch_one(query=username_query)
...
async def patch_user(
    ...
    try:
        ...
        record = UserReadSchema(**result).copy(
            update=record.dict(exclude_unset=True)
        )
    ...
    try:
        ...
        if result:
            if not pbkdf2_sha256.verify(
                record.old_password, result["password"]
            ):
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def request_reset_password(record: RequestResetPasswordSchema) -> dict:
    ...
    try:
        ...
        if email_otp_response.status_code == 200:
            query = (
                UserTable.update()
                .where(UserTable.c.email == record.email)
                .values(password_otp=otp_code, password_verified=False)
                .returning(UserTable)
            )
            await database.execute(query=query)
